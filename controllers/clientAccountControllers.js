...
export const getProfile = async (req, res) => {
  try {
    ...
    const data = {
      ...
      email:       user.email,
      phone:       user.mobile || "",
      location:    user.address?.city ? `${user.address.city}, ${user.address.state || ""}`.trim().replace(/,$/, "") : "",
      bio:         user.bio || "",
...
export const updatePersonal = async (req, res) => {
  try {
    ...
    const update = {
      ...
      ...(location     !== undefined && { "address.city": location }),
    };
...
export const updateHiringPreferences = async (req, res) => {
  try {
    const { freelancerLevel, budgetRange, communicationPreference, autoInviteFreelancers, jobVisibility } = req.body;
...
export const updatePaymentMethod = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(req.user._id, { $set: { "client.paymentMethod": paymentMethod } });
...
export const updateBillingAddress = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(req.user._id, { $set: { "client.billingAddress": billingAddress } });
  ...
  try {
    ...
    const user = await User.findByIdAndUpdate(
      req.user._id,
      {
        ...
        $push: { "client.paymentHistory": { amount: Number(amount), date: new Date(), status: "Success", description: "Wallet top-up" } },
      },
      { new: true }
    );

    return res.status(200).json({ success: true, data: { walletBalance: user.client.walletBalance } });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
export const updateNotifications = async (req, res) => {
  try {
    ...
    const allowed = ["proposalReceived", "hiringUpdates", "interviewReminders", "messageReceived", "paymentAlerts", "marketingEmails"];
    const update  = {};
    allowed.forEach((key) => {
      if (notifications[key] !== undefined) update[`client.notifications.${key}`] = Boolean(notifications[key]);
    });
...
export const sendDeleteOTP = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(req.user._id, { $set: { deleteOtp: otp, deleteOtpExpiry: expiry } });
...
export const deleteAccount = async (req, res) => {
  try {
    ...
    // Soft delete: free up email, block login
